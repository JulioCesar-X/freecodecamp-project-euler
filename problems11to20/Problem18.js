...
function maximumPathSumI(triangle) {
    ...
    const sumMaxOfPath = triangle[size - 1].slice();
    
    for (let row = size - 2; row >= 0; row--)
    {
        for (let route = 0;  route <= row; route++)
        {
            sumMaxOfPath[route] = triangle[row][route] + Math.max(sumMaxOfPath[route], sumMaxOfPath[route + 1])
        }
    }
    return sumMaxOfPath[0];
